...
function mainRotationMatrix( dim, axis1, axis2, angle )
	-- for the sake of the general rotation matrix this returns a homogeneous matrix of size dim+1
	local newMatrix = identity( dim+1 )
	newMatrix[axis1][axis1] = math.cos(angle)
	newMatrix[axis2][axis2] = math.cos(angle)
	newMatrix[axis1][axis2] = -math.sin(angle)
	newMatrix[axis2][axis1] = math.sin(angle)
...
function rotationMatrix( simplex, angle )
	...
	return M * mainRotationMatrix(n, n-1, n, angle) * MInv
end
